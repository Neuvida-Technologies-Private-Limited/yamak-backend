...
def is_user_validated_email(email: str) -> bool:
    """Checking if user is valided or not"""

    response = list(User.objects.filter(email=email).values())

    if len(response) == 0:
        raise UnexpectedRequestError(message='User not present')

    if not response[0].get('is_validated'):
        return False

    return True

def is_user_validated_phone(phone: str) -> bool:
    """Checking if user is valided or not"""

    response = list(User.objects.filter(phone=phone).values())
...
def create_otp_email(email: str) -> str:
    """
    Using a package pyotp to generate one time password
    The password will be saved to OneTimePassword
    """
...
def get_otp_email(email: str, otp: str) -> OneTimePassword:
    """Get OTP for a phone if not expired"""
...
def get_num_access_token(user):
    """Get total number of tokens issued for a user"""

    tokens = list(AccessToken.objects.filter(user=user).values())
